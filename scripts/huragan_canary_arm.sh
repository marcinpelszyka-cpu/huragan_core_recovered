...
BACKEND=${3:-${LIVE_SEND_BACKEND:-helius_sender}}
...
Environment=BUY_AMOUNT_SOL=${BUY_AMOUNT_SOL:-0.003}
...
Environment=AMM_MIN_POOL_SOL_FOR_ENTRY_LAMPORTS=${AMM_MIN_POOL_SOL_FOR_ENTRY_LAMPORTS:-500000000000}
...
Environment=HELIUS_SENDER_ENDPOINT=${HELIUS_SENDER_ENDPOINT:-http://fra-sender.helius-rpc.com/fast?swqos_only=true}
...
Environment=HELIUS_SENDER_MAX_PER_DAY=${HELIUS_SENDER_MAX_PER_DAY:-50}
Environment=MAX_DAILY_LOSS_SOL=${MAX_DAILY_LOSS_SOL:-0.01}
Environment=MAX_DAILY_TRADES=${MAX_DAILY_TRADES:-20}
Environment=MAX_CONSECUTIVE_LOSSES=${MAX_CONSECUTIVE_LOSSES:-3}
Environment=LIVE_RISK_MANAGER_ENABLED=true
EOF
