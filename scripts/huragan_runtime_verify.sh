...
printf 'effective='; systemctl show migration-sniper.service -p Environment --value \
  | tr ' ' '\n' \
  | grep -E '^(PAPER_MODE|LIVE_ARMED|LIVE_SEND_ENABLED|LIVE_AUTO_SELL_ENABLED|LIVE_SELL_SEND_ENABLED|ALLOW_PLAINTEXT_PRIVATE_KEY|AMM_LIVE_CANARY|PUMPPORTAL_ENABLED|MAX_TRADES_PER_RUN|BUY_AMOUNT_SOL|AMM_LIVE_BUY_MIN_OUT_BPS|LIVE_SEND_BACKEND|LIVE_SEND_PREFLIGHT_COMMITMENT)=' \
  | paste -sd ' ' - || true
printf 'env_file='; grep -E '^(PAPER_MODE|LIVE_ARMED|LIVE_SEND_ENABLED|LIVE_AUTO_SELL_ENABLED|LIVE_SELL_SEND_ENABLED|ALLOW_PLAINTEXT_PRIVATE_KEY|AMM_LIVE_CANARY|PUMPPORTAL_ENABLED|MAX_TRADES_PER_RUN|BUY_AMOUNT_SOL|AMM_LIVE_BUY_MIN_OUT_BPS|RPC_SEND_URL|LIVE_SEND_BACKEND|LIVE_SEND_PREFLIGHT_COMMITMENT)=' .env \
